...
function renderRow(c: Couche, state: DataState): string {
  ...
  return `
    <tr class="data-tr" data-couche-id="${c.id}">
      ...
      <td class="data-td-download">
        <button type="button" class="btn btn-sm btn-outline download-trigger" data-couche-id="${c.id}">
...
function renderDetailModal(c: Couche): string {
  ...
  return `
    <div class="admin-modal-overlay couche-modal-overlay" id="detail-modal">
      <div class="admin-modal admin-modal--couche" role="dialog" aria-modal="true">
        <div class="couche-modal-header">
          <h3>${c.nom_affichage}</h3>
          <button type="button" class="couche-modal-close" id="detail-modal-close" aria-label="Fermer">${icons.close}</button>
        </div>
        <div class="couche-modal-body">
          ${fields.map(f => `
            <div class="couche-detail-card">
              <span class="couche-detail-label">${f.label}</span>
              <span class="couche-detail-value">${f.value}</span>
            </div>
          `).join('')}
          <div class="couche-detail-card">
            <span class="couche-detail-label">Attributs</span>
            ${renderAttributs(c.attributs)}
          </div>
        </div>
        <div class="couche-modal-actions">
...
function renderAttributs(attributs: AttributDefinition[]): string {
  if (!attributs.length) return '<span class="couche-detail-value">-</span>'
  return `
    <div class="couche-attr-list">
      ${attributs.map(a => `
        <div class="couche-attr-item">
          <span class="couche-attr-name">${a.nom}</span>
          <span class="couche-attr-type ${ATTR_BADGE_CLASS[a.type.toLowerCase()] || ''}">${a.type}</span>
        </div>
      `).join('')}
    </div>
  `
...
function bindEvents(): void {
  ...
  pageRoot.querySelectorAll('.download-trigger').forEach(btn => {
    btn.addEventListener('click', () => {
