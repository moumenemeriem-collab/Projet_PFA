...
export function AdminMessagesPage(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const doFetch = async (): Promise<void> => {
      try {
        const data = await fetchAdminMessages(search, statut, currentPage)
        if (cancelled) return
  ...
  const handleReplySubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setSelectedMessage(msg)
      setReplyContenu('')
      showAlert('detail-alert', t('messages.reply_sent'))
    } catch (error) {
      showAlert('detail-alert', formatApiErrors(error), true)
    } finally {
      ...
      >
        ...
        <div className="msg-item-body">
          ...
          <div className="msg-item-meta">
            ...
            {!msg.est_lu ? <span className="msg-unread-label">{t('messages.unread')}</span> : null}
    ...
    <div className="msg-page">
      <div className="msg-page-header">
        <div>
          ...
          <p className="msg-page-desc">{t('messages.admin_desc')}</p>
        </div>
        <div className="msg-stats-row">
          <div className="msg-stat-card">
            <span className="msg-stat-value">{total}</span>
            <span className="msg-stat-label">{t('messages.total')}</span>
          </div>
          <div className="msg-stat-card msg-stat-card--unread">
            <span className="msg-stat-value">{nonLus}</span>
            <span className="msg-stat-label">{t('messages.unread_count')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="msg-toolbar">
        <div className="msg-search-field">
          ...
          <input
            ...
            placeholder={t('messages.search_admin_placeholder')}
            value={searchInput}
  ...
  const detailView = selectedMessage ? (
    <div className="msg-detail">
      ...
      <div className="msg-detail-message">
        <div className="msg-detail-author">
          ...
          <div>
            ...
            {selectedMessage.expediteur.telephone ? (
              <span className="msg-author-phone">{t('common.tel_label')} {selectedMessage.expediteur.telephone}</span>
            ) : null}
          </div>
        </div>
      ...
      {reponses.length > 0 ? (
        <div className="msg-replies">
          <h3 className="msg-replies-title">
            {icons.reply} {t('messages.replies_title')} ({reponses.length})
          </h3>
      ...
      <div className="msg-reply-form">
        <h3 className="msg-reply-form-title">{t('messages.reply_to')} {getFullName(selectedMessage.expediteur)}</h3>
        <div id="reply-alert" className={alertClass('reply-alert')} hidden={!alerts['reply-alert']}>
          {alerts['reply-alert']?.message ?? ''}
        </div>
        <form id="reply-form" noValidate onSubmit={(e) => { void handleReplySubmit(e) }}>
          <textarea
            id="reply-contenu"
          ...
          ></textarea>
          <button type="submit" className="btn btn-primary" disabled={replySending}>
            <span className="contact-submit-icon">{icons.send}</span> {t('messages.send_reply')}
          </button>
        </form>
      </div>
    </div>
  ...
  const replyEditModal = (
    ...
    >
      <div className="admin-modal" role="dialog" aria-modal="true">
        ...
        <form id="edit-reply-form" className="admin-modal-form" noValidate onSubmit={(e) => { void handleReplyModalSubmit(e) }}>
          <div id="edit-reply-error" className="form-alert form-alert--error" hidden={!replyEditError}>
            {replyEditError}
          </div>
          <div className="form-field">
            <label htmlFor="edit-reply-contenu" className="form-label">{t('messages.content')}</label>
            <textarea
  ...
  } else if (view === 'detail' && selectedMessage) {
    content = detailView
  } else {
    content = listView
  }
  ...
  return (
    <DashboardLayout role="admin" activePage="messages">
      {content}
